...
count-=opts.by}}function*countForwardGenerator(opts){var count=opts.from;if(opts.from<=opts.to)while(count<=opts.to){yield count;count+=opts.by}else while(count>=opts.to){yield count;count-=opts.by}}function*countGenerator(opts){if(opts.reversed)yield*countReverseGenerator(opts);else yield*countForwardGenerator(opts)}function CounterGenerator(){if(!(this instanceof CounterGenerator))return new CounterGenerator;var opts={reversed:false,from:0,to:Number.MAX_SAFE_INTEGER,by:1},iterator;_.setValue(this,
"state",function(){return _.assign({},opts)});_.setValue(this,Symbol.iterator,function(){if(!iterator)iterator=countGenerator(_.assign({},opts));return iterator});_.setValue(this,"from",function(number){opts.from=_.clampToSafeIntegerRange(number);return this});_.setValue(this,"to",function(number){opts.to=_.clampToSafeIntegerRange(number);return this});_.setValue(this,"by",function(number){opts.by=Math.abs(_.clampToSafeIntegerRange(number));if(!opts.by)throw new TypeError("can not count by zero");
...
var length=_.isArrayLike(subject)?subject.length:0,opts=_.assign({length:length,reversed:false,from:0,to:length-1,by:1},$.OPTS.ENTRIES),iterator;_.setValue(this,"state",function(){return _.assign({},opts)});_.setValue(this,Symbol.iterator,function(){if(!iterator)iterator=arrayGenerator(subject,_.assign({},opts));return iterator});_.setValue(this,"entries",function(){_.assign(opts,$.OPTS.ENTRIES);return this});_.setValue(this,"values",function(){_.assign(opts,$.OPTS.VALUES);return this});_.setValue(this,
"keys",function(){_.assign(opts,$.OPTS.KEYS);return this});_.setValue(this,"reverse",function(){opts.reversed=!opts.reversed;return this});_.setValue(this,"slice",function(start,end){_.setIndexesOpts(start,end,opts);return this})}return ArrayGenerator}(),StringGenerator:function(){function getStringYieldValue(opts,character,key){var value,result;if(opts.keys)result=key;else{value=String.fromCodePoint(character.codePointAt());if(opts.values)result=value;else result=[key,value]}return result}function*stringGenerator(subject,
opts){var generator,next,char1,char2,key;if(!opts.length)return;next=true;generator=g.CounterGenerator(opts);generator.from(opts.from).to(opts.to).by(opts.by);_.setReverseIfOpt(opts,generator);for(key of generator)if(next)if(opts.reversed){char1=subject[key-1];char2=subject[key];next=!_.isSurrogatePair(char1,char2);if(next)yield getStringYieldValue(opts,char2,key)}else{char1=subject[key];char2=subject[key+1];next=!_.isSurrogatePair(char1,char2);yield getStringYieldValue(opts,char1+char2,key)}else{next=
!next;if(opts.reversed)yield getStringYieldValue(opts,char1+char2,key)}}function StringGenerator(subject){if(!(this instanceof StringGenerator))return new StringGenerator(subject);var length=_.isArrayLike(subject)?subject.length:0,opts=_.assign({length:length,reversed:false,from:0,to:length-1,by:1},$.OPTS.ENTRIES),iterator;_.setValue(this,"state",function(){return _.assign({},opts)});_.setValue(this,Symbol.iterator,function(){if(!iterator)iterator=stringGenerator(subject,_.assign({},opts));return iterator});
_.setValue(this,"entries",function(){_.assign(opts,$.OPTS.ENTRIES);return this});_.setValue(this,"values",function(){_.assign(opts,$.OPTS.VALUES);return this});_.setValue(this,"keys",function(){_.assign(opts,$.OPTS.KEYS);return this});_.setValue(this,"reverse",function(){opts.reversed=!opts.reversed;return this});_.setValue(this,"slice",function(start,end){var char1,char2;_.setIndexesOpts(start,end,opts);if(opts.from){char1=subject[opts.from-1];char2=subject[opts.from];if(_.isSurrogatePair(char1,
char2))opts.from+=1}if(opts.to){char1=subject[opts.to-1];char2=subject[opts.to];if(_.isSurrogatePair(char1,char2))opts.to-=1}return this})}return StringGenerator}(),EnumerateGenerator:function(){function*enumerateGenerator(subject,opts){for(var key in subject)if(!opts.own||_.hasOwn(subject,key))yield _.getYieldValue(opts,subject,key)}function EnumerateGenerator(subject){if(!(this instanceof EnumerateGenerator))return new EnumerateGenerator(subject);var opts=_.assign({own:false},$.OPTS.ENTRIES),iterator;
_.setValue(this,"state",function(){return _.assign({},opts)});_.setValue(this,Symbol.iterator,function(){if(!iterator)iterator=enumerateGenerator(subject,_.assign({},opts));return iterator});_.setValue(this,"entries",function(){_.assign(opts,$.OPTS.ENTRIES);return this});_.setValue(this,"values",function(){_.assign(opts,$.OPTS.VALUES);return this});_.setValue(this,"keys",function(){_.assign(opts,$.OPTS.KEYS);return this});_.setValue(this,"own",function(){opts.own=!opts.own;return this})}return EnumerateGenerator}(),
repeatGenerator:function*(subject){do yield subject;while(true)}};_.populatePrototypes();return function(){function makeCounterGenerator(subject,to,by){var generator=g.CounterGenerator();if(_.isNumber(subject)){if(_.isNil(to))generator.to(subject);else generator.from(subject).to(to);if(!_.isNil(by))generator.by(by)}return generator}Reiterate=function(subject,to,by){if(!(this instanceof Reiterate))return new Reiterate(subject,to,by);var generator;if(_.isNil(subject)||_.isNumber(subject))generator=
